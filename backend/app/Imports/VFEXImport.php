...
class VFEXImport implements ToModel, WithStartRow
{
    /**
    * @param array $row
    *
...
//            dd($row[1]);
            ...
            $company = VFEX::where('symbol', Str::lower(trim($row[1])))->first();
//            dd(trim($row[1]));
            ...
            $count = VFEXPriceSheet::where('date', $ymd)->where('v_f_e_x_e_s_id', $companyId)->count();
            ...
            if($count < 1) {
                ...
                return new VFEXPriceSheet([
                    'v_f_e_x_e_s_id'  => $companyId,
                    'volume'    => $volume,
